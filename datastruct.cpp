...
bool OutRaster::copyxy(Raster r1, Raster r2){
	if(r1.x.size()!=r2.x.size() ||
		...
		cerr << "OPTIONAL: Enter 'r1' or 'r2' to use input raster 1 or 2 as a template output raster:" << endl;
		string rasterpref;
		getline(cin, rasterpref);
		transform(rasterpref.begin(), rasterpref.end(), rasterpref.begin(), ::tolower);
		if(rasterpref.compare("r1")==0){
			OutRaster::x = r1.x;
			OutRaster::y = r1.y;
		}
		if(rasterpref.compare("r1")==0){
	...
	if(r1.ulx!=r2.ulx || r1.uly!=r2.uly){
		cerr << "ERROR: Upper-left X or Y coordinates do not match. Unable to copy to output raster." << endl;
...
// 		float mu1 = t1.val[i];
// 		float mu2 = t2.val[i];
// 		float sd1 = t1e.val[i]/1.96;
// 		float sd2 = t2e.val[i]/1.96;
//
// 		// option 2: vectors
...
// //				normal_distribution<float> dist1(0.0, sd1);
// //				normal_distribution<float> dist2(0.0, sd2);
// //
...
// //
// //				// option 1: update single value
// //				chng.push_back((mu2 + er2) - (mu1 + er1));
// //
...
//
//
// 		////// UPDATE CHANGE VALUES AND PROBABILITY OF CHANGE //////
// 		if(mu1 > -999 && mu2 > -999){
//       cout << "testing 1:" << t1.val[i] << " -- " << t2.val[i] << endl;
// //			OutRaster::val[i] = (accumulate(chng.begin(), chng.end(), 0.0))/iter;
// 			OutRaster::val[i] = mu2 - mu1;
//
// 			// Compute the probability of intersection as the integral of the area under BOTH error distributions
// 			if(mu1 < mu2){
// 				// compute intersection point of the two curves
// 				float c = ((mu2*(pow(2.0,sd1)))-(sd2*((mu1*sd2)+(sd1+sqrt((pow(2.0,mu1-mu2))+(2*(pow(2.0,sd1)-pow(2.0,sd2))*log((sd1/sd2))))))))/(pow(2.0,sd1)-(sd2));
//
// 				// assign probability raster value
//         //OutRaster::prob[i] = 1-(1-(0.5*erf((c-mu1)/(sqrt(2)*sd1)))+(0.5*erf((c-mu2)/(sqrt(2)*sd1))));  // OLD WAY -> WRONG
// 				OutRaster::prob[i] = (1-(0.5*erf((c-mu1)/(sqrt(2)*sd1)))+(0.5*erf((c-mu2)/(sqrt(2)*sd1))));
//
//         // compute z score
//         //OutRaster::z_score_value[i] = 1;
//         // OutRaster::z_score_value[i] = (mu2-mu1)/(sqrt(pow(2.0,sd1)+pow(2.0,sd2)));
// 			} else if(mu2 < mu1){
// 				// because it matters which distribution is lower (in value) than the other:
// 				// if mu2 is LESS THAN mu1 (t2 < t1), we must swap mu and sd values for computations
// 				// swap mu1 and mu2;
// 				//float mu1a = mu2; // WORKS
// 				//mu2 = mu1; // WORKS
// 				//mu1 = mu1a; // WORKS
//
// 				// sqap sd1 and sd2;
// 				//float sd1a = sd2; // WORKS
// 				//sd2 = sd1; // WORKS
// 				//sd1 = sd1a; // WORKS
//
// 				// compute intersection point of the two curves
// //				float c = ((mu2*(sd1^2))-(sd2*((mu1*sd2)+(sd1+sqrt(((mu1-mu2)^2)+(2*((sd1^2)-(sd2^2))*log((sd1/sd2))))))))/((sd1^2)-(sd2));
// 				//float c = ((mu2*(pow(2.0,sd1)))-(sd2*((mu1*sd2)+(sd1+sqrt((pow(2.0,mu1-mu2))+(2*(pow(2.0,sd1)-pow(2.0,sd2))*log((sd1/sd2))))))))/(pow(2.0,sd1)-(sd2)); // WORKS
//         float c = ((mu1*(pow(2.0,sd2)))-(sd1*((mu2*sd1)+(sd2+sqrt((pow(2.0,mu2-mu1))+(2*(pow(2.0,sd2)-pow(2.0,sd1))*log((sd2/sd1))))))))/(pow(2.0,sd2)-(sd1));
//
// 				// assign probability raster value
// 				//OutRaster::prob[i] = 1-(1-(0.5*erf((c-mu1)/(sqrt(2)*sd1)))+(0.5*erf((c-mu2)/(sqrt(2)*sd1))));
//         //OutRaster::prob[i] = 1-(1-(0.5*erf((c-mu1)/(sqrt(2)*sd1)))+(0.5*erf((c-mu2)/(sqrt(2)*sd1))));  // OLD WAY -> WRONG
//         OutRaster::prob[i] = (1-(0.5*erf((c-mu2)/(sqrt(2)*sd2)))+(0.5*erf((c-mu1)/(sqrt(2)*sd2)))); // WORKS
//
//         // compute z score
//         //OutRaster::z_score_value[i] = 1;
//         // OutRaster::z_score_value[i] = (mu2-mu1)/(sqrt(pow(2.0,sd1)+pow(2.0,sd2)));
...
// //			}
// //
// ////		} else if(p_change == 0 && mu1 > -999 && mu2 > -999){
// ////			OutRaster::prob[i] = 0;
  ...
  // fill(OutRaster::z_score_value.begin(), OutRaster::z_score_value.end(), -99999);
	...
	for(size_t i=0; i<OutRaster::val.size(); i++)
	{
      ...
      //cout << "testing 1:" << t1.val[i] << " -- " << t2.val[i] << endl;
        ...
        // NOTE: mu1 and mu2 (and corresponding standard deviations) have been flipped to account
        // for surface at t2 below surface at t1
